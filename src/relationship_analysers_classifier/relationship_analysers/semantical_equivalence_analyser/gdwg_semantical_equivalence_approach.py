...
from prompt_configs.equivalence.equivalence_prompts_llm import build_prompt
...
import dotenv, os, tiktoken, math

dotenv.load_dotenv()
    
class SemanticEquivalenceAnalyserGdwg(RelationResolver):    
    def __init__(self, model: str = os.getenv("MODEL_GDWG"), typ: TypeGraphUs = TypeGraphUs.ENTITY) -> None:
        """
        Initialize the GDWG generalisation analyzer.
        
        Args:
            typ (TypeGraphUs): Type of element - ENTITY, ACTION, or PERSONA
            model (str): GDWG model to use. Defaults to gpt-3.5-turbo
        """
        self.model = model
        self.client = OpenAI(
            api_key=os.getenv("MODEL_KEY_GDWG"),
            base_url=os.getenv("GDWG_API_URL")
        )
        ...
        self.url = os.getenv("GDWG_API_URL")
        ...
        tokens: int = int(os.getenv("GDWG_AI_MODEL_CONTEXT_WINDOWS"))
    ...
    def are_contained(self, pairs: set[tuple[str, str]]) -> list[tuple[str, str, bool]]:
        ...
        while len(pairs_to_process) > 0:
            ...
            while len(pairs_to_process) > 0 and len(test_batch) <= self.max_batch_size:
                ...
                prompt = build_prompt(test_batch, self.typ)
            ...
            if current_batch:
                prompt = build_prompt(current_batch, self.typ)
