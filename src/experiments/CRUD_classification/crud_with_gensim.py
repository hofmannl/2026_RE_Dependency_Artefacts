...
m: GensimWord2VecCRUDClassifier = GensimWord2VecCRUDClassifier(1)
...
end_time = time.time()
print(f"Gensim | {file_name} | Mode 1 word with action | {end_time - start_time} seconds")

with open(current_file.parents[0] / f"{file_name}_crud_classification_gensim_word_results.csv", "w", encoding="utf-8") as f:
...
end_time = time.time()
print(f"Gensim | {file_name} | Mode 1 word with sentences | {end_time - start_time} seconds")

with open(current_file.parents[0] / f"{file_name}_crud_classification_gensim_sentence_results.csv", "w", encoding="utf-8") as f:
    f.write("story|action|classified_action|similarity\n")
    for r in results:
        f.write(f'{r[0]}|{r[1]}|{r[2].value}|{r[3]}\n')