...
m: FastTextCrudAnalyserClassifier = FastTextCrudAnalyserClassifier(1)
...
end_time = time.time()
print(f"FastText | {file_name} | Mode 1 words with action | {end_time - start_time} seconds")

with open(current_file.parents[0] / f"{file_name}_crud_classification_fasttext_1_words_results.csv", "w", encoding="utf-8") as f:
...
end_time = time.time()
print(f"FastText | {file_name} | Mode 1 words with sentences | {end_time - start_time} seconds")

with open(current_file.parents[0] / f"{file_name}_crud_classification_fasttext_sentence_results.csv", "w", encoding="utf-8") as f:
    f.write("story|action|classified_action|similarity\n")
    for r in results:
        f.write(f'{r[0]}|{r[1]}|{r[2].value}|{r[3]}\n')