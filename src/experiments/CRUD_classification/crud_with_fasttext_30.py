...
from relationship_analysers_classifier.crud_classifier.fasttext_analyser_similarity import FastTextCrudAnalyserClassifier
...
m: FastTextCrudAnalyserClassifier = FastTextCrudAnalyserClassifier(bag_of_words_size=30)
...
end_time = time.time()
print(f"FastText | {file_name} | Mode 30 words with action | {end_time - start_time} seconds")

with open(current_file.parents[0] / f"{file_name}_crud_classification_fasttext_30_words_results.csv", "w", encoding="utf-8") as f:
...
end_time = time.time()
print(f"FastText | {file_name} | Mode 30 words with sentences | {end_time - start_time} seconds")

with open(current_file.parents[0] / f"{file_name}_crud_classification_fasttext_30_words_sentence_results.csv", "w", encoding="utf-8") as f:
    f.write("story|action|classified_action|similarity\n")
    for r in results:
        f.write(f'{r[0]}|{r[1]}|{r[2].value}|{r[3]}\n')