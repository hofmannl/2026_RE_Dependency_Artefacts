...
### Personas
m = SemanticEquivalenceAnalyserOpenAI(typ=TypeGraphUs.PERSONA)

start_time = time.time()
total_results = m.compute_relations(persona_pairs)
end_time = time.time()

print(f"Semantic Equivalence Analyser OpenAI-{os.getenv("OPENAI_MODEL")} | {file_name} | Persona pairs | {end_time - start_time} seconds")
save_results_to_csv(
    current_file.parents[0] / f"{file_name}_semantic_equivalence_persona_model_{os.getenv("OPENAI_MODEL")}.csv", 
    total_results, 
...
### Actions
m = SemanticEquivalenceAnalyserOpenAI(typ=TypeGraphUs.ACTION)

start_time = time.time()
total_results = m.compute_relations(action_pairs)
end_time = time.time()

print(f"Semantic Equivalence Analyser OpenAI-{os.getenv("OPENAI_MODEL")} | {file_name} | Action pairs | {end_time - start_time} seconds")
save_results_to_csv(
    current_file.parents[0] / f"{file_name}_semantic_equivalence_action_model_{os.getenv("OPENAI_MODEL")}.csv", 
    total_results, 
...
### Entities
m = SemanticEquivalenceAnalyserOpenAI(typ=TypeGraphUs.ENTITY)

start_time = time.time()
total_results = m.compute_relations(entity_pairs)
end_time = time.time()

print(f"Semantic Equivalence Analyser OpenAI-{os.getenv("OPENAI_MODEL")} | {file_name} | Entity pairs | {end_time - start_time} seconds")
save_results_to_csv(
    current_file.parents[0] / f"{file_name}_semantic_equivalence_entity_model_{os.getenv("OPENAI_MODEL")}.csv", 
    total_results, 
