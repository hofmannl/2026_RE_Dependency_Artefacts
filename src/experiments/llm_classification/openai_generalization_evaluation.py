import os
from relationship_analysers_classifier.relationship_analysers.generalisation_analyser.openai_generalisation_approach import GeneralisationAnalyserOpenAI
...
### Personas
m = GeneralisationAnalyserOpenAI(TypeGraphUs.PERSONA)

start_time = time.time()
total_results = m.compute_relations(entity_pairs)
end_time = time.time()

print(f"Containment Analyser OpenAI-{os.getenv("OPENAI_MODEL")} | {file_name} | Persona pairs | {end_time - start_time} seconds")
save_results_to_csv(
    current_file.parents[0] / f"{file_name}_containment_persona_model_{os.getenv("OPENAI_MODEL")}.csv", 
    total_results, 
...
### Actions
m = GeneralisationAnalyserOpenAI(TypeGraphUs.ACTION)

start_time = time.time()
total_results = m.compute_relations(entity_pairs)
end_time = time.time()

print(f"Containment Analyser OpenAI-{os.getenv("OPENAI_MODEL")} | {file_name} | Action pairs | {end_time - start_time} seconds")
save_results_to_csv(
    current_file.parents[0] / f"{file_name}_containment_action_model_{os.getenv("OPENAI_MODEL")}.csv", 
    total_results, 
...
### Entities
m = GeneralisationAnalyserOpenAI(TypeGraphUs.ENTITY)

start_time = time.time()
total_results = m.compute_relations(entity_pairs)
end_time = time.time()

print(f"Containment Analyser OpenAI-{os.getenv("OPENAI_MODEL")} | {file_name} | Entity pairs | {end_time - start_time} seconds")
save_results_to_csv(
    current_file.parents[0] / f"{file_name}_containment_entity_model_{os.getenv("OPENAI_MODEL")}.csv", 
    total_results, 
