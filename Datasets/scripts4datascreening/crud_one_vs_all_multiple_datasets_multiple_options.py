...
for gt_path, pred_path in file_pairs:
    ...
    for idx, pred_row in results_df.iterrows():
        ...
        for _, gt_row in ground_truth_df.iterrows():
            ...
            if (gt_row['story'] in pred_row['story'] and pred_row['action'] == gt_row['action']):
                ...
                # Check if story-action pair already exists in matches
                existing_match = next((m for m in matches if m['story'] == match['story'] and m['action'] == match['action']), None)
                if existing_match is None:
                    matches.append(match)
                else:
                    if pred_row['classified_action'] == gt_row['classified_action']:
                    # Replace with the new occurrence (ground truth takes precedence)
                        existing_match['classified_action_x'] = pred_row['classified_action']
                        existing_match['classified_action_y'] = gt_row['classified_action']
    ...
    for idx, (pred, true) in enumerate(zip(y_pred, y_true)):
        if pred != true:
            mismatch = {
                'story': eval_df.iloc[idx]['story'],
                'action': eval_df.iloc[idx]['action'],
                'predicted_action': pred,
                'true_action': true,
                'file': filename_identifier
            }
    ...
    temp = eval_df[['story', 'action', 'classified_action_y']].rename(columns={'classified_action_y': 'classified_action'})
    temp.to_csv(Path(__file__).parent / f'used_gt_for_{filename_identifier}.csv', index=False, sep=separator)
...
precision, recall, f1, support = precision_recall_fscore_support(all_y_true, all_y_pred, average=None, labels=classes, zero_division=0)
...
print("\n" + classification_report(all_y_true, all_y_pred, labels=classes, zero_division=0))
...
# Save incorrectly predicted samples to a file
if all_mismatches:
    mismatches_df = pd.DataFrame(all_mismatches)
    mismatches_path = Path(__file__).parent / f'incorrectly_predicted_samples.csv'
    mismatches_df.to_csv(mismatches_path, index=False)
    print(f"\nIncorrectly predicted samples saved to: {mismatches_path}")
    print(f"Total incorrectly predicted samples: {len(all_mismatches)}")
else:
    print("\nNo incorrectly predicted samples found!")
