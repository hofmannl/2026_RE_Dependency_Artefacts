...
def extract_persona_pairs(json_file, output_file):
    """
    Extract all possible pairs between personas from user stories.
    Generates all unique combinations of personas that appear in the data.
    """
    ...
    personas_list = sorted(list(all_personas))
    
    print(personas_list)
    
    # generates pairs like A|A, A|B, B|A, B|B, etc.
    persona_pairs = list(
        itertools.product(
                personas_list, 
                personas_list
    ...
    #     for persona in personas:
    #         if persona:
    #             lc_persona = strip_lowercase_lemmatize_word(persona)
    ...
    persona_pairs = sorted(list(real_appearances)) 

    df: pd.DataFrame = pd.DataFrame(persona_pairs, columns=['Persona 1', 'Persona 2'])
    df.to_csv(output_file, index=False, encoding='utf-8', sep='|')
    
    print(f"CSV file created: {output_file}")
    print(f"Total unique personas: {len(personas_list)}")
...
if __name__ == "__main__":
    ...
    arg_two = r"D:\_Projects\_myProjects\dependencyanalyseandsynergyofuserstories\Datasets\elaborated_ground_truth\datasets\annotated\extracted_informations\g03_baseline_persona_pairs.csv"
    ...
    try:
        extract_persona_pairs(input_file, output_file)
...
# --- G08 Baseline Example Paths --- #
    ...
    arg_two = r"D:\_Projects\_myProjects\dependencyanalyseandsynergyofuserstories\Datasets\elaborated_ground_truth\datasets\annotated\extracted_informations\g08_baseline_persona_pairs.csv"
    ...
    arg_two = r"D:\_Projects\_myProjects\dependencyanalyseandsynergyofuserstories\Datasets\elaborated_ground_truth\datasets\annotated\extracted_informations\g17_baseline_persona_pairs.csv"
    ...
    arg_two = r"D:\_Projects\_myProjects\dependencyanalyseandsynergyofuserstories\Datasets\elaborated_ground_truth\datasets\annotated\extracted_informations\g22_baseline_persona_pairs.csv"
